...
#ifdef SPLASH_SCREEN
  while (readButton(btnPlay) == HIGH &
         readButton(btnStop) == HIGH &
         readButton(btnUp)   == HIGH &
         readButton(btnDown) == HIGH &
         readButton(btnRoot) == HIGH)
  {
...
//digitalRead wrapper for LCD Keypad Shield
int readButton(int button)
{
#ifdef LCD_KEYPAD
  
  int readBtn = 9999;
  int adc_key = analogRead(0);      // read pressed key from ADC0

  //read values can differ from your shield, if unable to read buttons change values.

  if (adc_key > 1000) readBtn = 9999;         // No pressed button
  if (adc_key < 50)   readBtn = btnRoot;      //right
  else if (adc_key < 150)  readBtn = btnUp;   //up
  else if (adc_key < 350)  readBtn = btnDown; //down
  else if (adc_key < 550)  readBtn = btnStop; //left
  else if (adc_key < 750)  readBtn = btnPlay; //select
...
void loop(void) {
  ...
  motorState = readButton(btnMotor);
#if (SPLASH_SCREEN && TIMEOUT_RESET)
  ...
  if (millis() - timeDiff > 50) {   // check switch every 50ms
    ...
    if (readButton(btnRoot) == LOW && start == 1 && pauseOn == 1
#ifdef btnRoot_AS_PIVOT
        && readButton(btnStop) == LOW
#endif
       ) {            // change polarity
    ...
    lastbtn = false;
    if (readButton(btnRoot) == LOW && start == 0 && !lastbtn) {                                     // show min-max dir
      //if(readButton(btnRoot)==LOW && start==0 && readButton(btnStop)==LOW ){
...
#endif
#endif
      while (readButton(btnRoot) == LOW && !lastbtn) {
        //prevent button repeats by waiting until the button is released.
        //delay(50);
        lastbtn = 1;
        checkLastButton();
    ...
    if (readButton(btnRoot) == LOW && start == 1 && pauseOn == 1 && !lastbtn) {                                   // show min-max block
      //if(readButton(btnRoot)==LOW && start==1 && pauseOn==1 && readButton(btnStop)==LOW ){
      /*
              lcd.setCursor(0,0);
              lcd.print(BAUDRATE);
              lcd.print(' ');
      ...
      */
      ...
      while (readButton(btnRoot) == LOW && start == 1 && !lastbtn) {
        //prevent button repeats by waiting until the button is released.
        //delay(50);
        lastbtn = 1;
        checkLastButton();
    ...
    if (readButton(btnRoot) == LOW && start == 0
#ifdef btnRoot_AS_PIVOT
        && readButton(btnStop) == LOW
#endif
       ) {                  // go menu
    ...
    if (readButton(btnStop) == LOW && start == 1
#ifdef btnRoot_AS_PIVOT
        && readButton(btnRoot)
#endif
       ) {
    ...
    if (readButton(btnStop) == LOW && start == 0 && subdir > 0) {         // back subdir
#if (SPLASH_SCREEN && TIMEOUT_RESET)
      timeout_reset = TIMEOUT_RESET;
#endif
      fileName[0] = '\0';
...
#ifdef BLOCKMODE
    if (readButton(btnUp) == LOW && start == 1 && pauseOn == 1
#ifdef btnRoot_AS_PIVOT
        && readButton(btnRoot)
#endif
    ...
    if (readButton(btnUp) == LOW && start == 1 && pauseOn == 1 && readButton(btnRoot) == LOW) { // up block half-interval search
    ...
    if (readButton(btnUp) == LOW && start == 0
#ifdef btnRoot_AS_PIVOT
        && readButton(btnRoot)
#endif
      ...
      /*
             ...
             while ((readButton(btnUp)==LOW) && (millis() - timeDiff2 < 200)) {
               //prevent button repeats by waiting until the button is released.
               delay(50);
             }
      */
...
#ifdef btnRoot_AS_PIVOT
    if (readButton(btnUp) == LOW && start == 0 && readButton(btnRoot) == LOW) { // up dir half-interval search
#if (SPLASH_SCREEN && TIMEOUT_RESET)
      timeout_reset = TIMEOUT_RESET;
#endif
      //Move up a file in the directory
...
#endif
#ifdef BLOCKMODE
    if (readButton(btnDown) == LOW && start == 1 && pauseOn == 1
#ifdef btnRoot_AS_PIVOT
        && readButton(btnRoot)
#endif
...
#endif
#if defined(BLOCKMODE) && defined(btnRoot_AS_PIVOT)
    if (readButton(btnDown) == LOW && start == 1 && pauseOn == 1 && readButton(btnRoot) == LOW) { // down block half-interval search
    ...
    if (readButton(btnDown) == LOW && start == 0
#ifdef btnRoot_AS_PIVOT
        && readButton(btnRoot)
#endif
      ...
      /*
             timeDiff2 = millis();           // get current millisecond count
             while ((readButton(btnDown)==LOW) && (millis() - timeDiff2 < 200)) {
               //prevent button repeats by waiting until the button is released.
               delay(50);
             }
      */
...
#ifdef btnRoot_AS_PIVOT
    if (readButton(btnDown) == LOW && start == 0 && readButton(btnRoot) == LOW ) {       // down dir half-interval search
#if (SPLASH_SCREEN && TIMEOUT_RESET)
      timeout_reset = TIMEOUT_RESET;
#endif
      //Move down a file in the directory
...
void debouncemax(int boton) {
  timeDiff2 = millis();
  while ((readButton(boton) == LOW) && (millis() - timeDiff2 < 200)) {
    //prevent button repeats by waiting until the button is released.
    delay(50);
  }
}
